...
MODULOS = {
    ...
    "FIN": {"icon":"💵","full":"Finanzas","tag":"tag-fin",
            "subs":[("💵","Cotizador","Tarifas"),("📊","P&G por Ruta","Rentabilidad"),("🧾","Documentos","Remisiones")]},
...
# ── WMS Inventario ─────────────────────────────────────────────
    ...
    with f1:
        ...
        qs = st.number_input("Cantidad", 1, cfg("cap_almacen"), key="inv_qty")
        ...
        if st.button("Registrar", use_container_width=True):
            ...
            elif ts=="entrada" and stk+qs>cfg("cap_almacen"): alerta("err","Supera capacidad.")
            else:
...
# ── CLIENTES — Stock Clientes ──────────────────────────────────
    ...
    with t2:
        ...
        with c1:
            ...
            a_cantidad  = st.number_input("Cantidad",0.0,100000.0,100.0,step=1.0,key="a2_qty")
...
# ── CLI Perecederos ────────────────────────────────────────────
    ...
    with t2:
        ...
        with c1:
            ...
            qty_l = st.number_input("Cantidad (unidades/kg)",1,cfg("cap_almacen"),100,key="lq")
            ...
            if not ord_df.empty and "numero" in ord_df.columns:
                ...
                if st.button("Actualizar Estado",key="os_upd_btn"):
                    try:
                        ...
                        alerta("ok",f"Orden <b>{ord_sel}</b> → <b>{new_est}</b>"); st.rerun()
                    except Exception as e: alerta("err",f"Error: {e}")
    with t2:
        ...
        with c1:
            ...
            if tipo_key == "recoleccion":
                ...
                os_qty = st.number_input("Cantidad",1,100000,500,key="os_qty")
                ...
                os_qty = st.number_input("Cantidad (unidades)",1,10000,168,key="os_qty_f")
                os_unidad = "unidades"
                ...
                os_qty = st.number_input("Cantidad (unidades)",1,cfg("cap_almacen"),168,key="os_qty_t")
                os_unidad = "unidades"
                ...
                os_qty = st.number_input("Cantidad (unidades)",1,10000,168,key="os_qty_e")
    ...
    with t2:
        ...
        with c2:
            ...
            if pc_tipo_servicio in ["Transporte (flete)","Transporte + Almacenamiento","Recolección en origen"]:
                ...
                pc_qty  = st.number_input("Cantidad a transportar",1,10000,168,key="pc_qty")
...
# ── FIN Cotizador ──────────────────────────────────────────────
    ...
    with c1:
        ...
        qty_c = st.number_input("Unidades",1,10000,168,key="cot_qty")
...
# ── FIN Documentos (+ Historial integrado) ─────────────────────
    ...
    with t2:
        ...
        with c2_:
            ...
            iva_ = int(tot_mg_fac*.19); tot_ = tot_mg_fac+iva_
            k1_,k2_,k3_ = st.columns(3)
            k1_.metric("Subtotal",f"${sub_:,}")
            k2_.metric(f"Margen {cfg('margen_pct')}% + IVA",f"${mg_fac+iva_:,}")
            k3_.metric("TOTAL",f"${tot_:,}")
...
# ── SYS Configuración ─────────────────────────────────────────
    ...
    with t3:
        c1_,c2_ = st.columns(2)
        with c1_: st.session_state["cfg_cap_almacen"]=st.number_input("Cap. máx. (uds)",100,50000,int(cfg("cap_almacen")),step=100,key="cfg_ca")
        with c2_: st.session_state["cfg_stock_min"]=st.number_input("Stock mínimo/CEDI",0,500,int(cfg("stock_min")),key="cfg_sm")
    with t4:
        c1_,c2_ = st.columns(2)
