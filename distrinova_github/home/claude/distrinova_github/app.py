...
def reset_sistema():
    db = supabase()
    tablas_borrar = ["ordenes_servicio","pedidos_comerciales","despachos","movimientos","lotes","ia_chat","almacenamiento_clientes","facturas"]
    tablas_limpiar_legacy = ["pedidos"]
    for t in tablas_borrar + tablas_limpiar_legacy:
        try: db.table(t).delete().gt("id", 0).execute()
        except: pass
    # Reiniciar stock a valores iniciales de simulacion
    for cedi in CEDIS:
    ...
    with t1:
        if ord_df.empty:
            ...
            if not ord_df.empty and "numero" in ord_df.columns:
                ...
                with col_act:
                    btn_actualizar = st.button("✏️ Actualizar Estado", key="os_upd_btn", use_container_width=True)
                with col_del:
                    btn_eliminar = st.button("🗑️ Eliminar Orden", key="os_del_btn", use_container_width=True)
                if btn_eliminar:
                    try:
                        supabase().table("ordenes_servicio").delete().eq("numero", ord_sel).execute()
                        alerta("warn", f"Orden <b>{ord_sel}</b> eliminada permanentemente.")
                        st.rerun()
                    except Exception as e: alerta("err", f"Error al eliminar: {e}")
                if btn_actualizar:
                    try:
                        supabase().table("ordenes_servicio").update({"estado":new_est}).eq("numero",ord_sel).execute()
                        # Registrar ingreso automáticamente al entregar (solo si no existe ya)
                        if new_est == "entregado":
                            ...
                            if not row_os.empty:
                                ...
                                try:
                                    # ✅ Verificar si ya existe factura para esta OS (evitar duplicados)
                                    fac_exist = supabase().table("facturas").select("id").eq("ruta", ord_sel).execute()
                                    ya_existe = fac_exist.data and len(fac_exist.data) > 0
                                    if not ya_existe:
                                        ...
                                        alerta("warn",f"Orden <b>{ord_sel}</b> → <b>Entregado</b> ✅ (Ingreso ya estaba registrado, no se duplicó)")
                                except:
